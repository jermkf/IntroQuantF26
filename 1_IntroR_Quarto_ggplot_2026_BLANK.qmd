...
THIS TUTORIAL COMPLETED BY: Jeremy Flores
...
sweetness <- ggplot(candy_rankings) + 
  aes(x =sugarpercent , y =winpercent ) + geom_point()
...
```{r}
sweetness <- ggplot(candy_rankings)+ aes(x=chocolate, y=sugarpercent)+ geom_boxplot()
sweetness
```
...
```{r}
sweethist <- ggplot(candy_rankings) + aes(x=sugarpercent)+geom_histogram()
sweethist
```
